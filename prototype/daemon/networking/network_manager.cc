...
void NetworkManager::subscribe_to_packet(prototype::protobuf::FvPacketType type, SubCb &&handler,
                                         bool task) {
    handlers_[type].emplace_front(make_pair(task, handler));
}
    ...
    while (task_processor_active_.load()) {
        {
            ...
            for (int i = 0; i < nonblocking_tasks_.size(); i++) {
                auto status = nonblocking_tasks_[i].wait_for(std::chrono::seconds(0));
...
void NetworkManager::poll_network(bool block_completion) {
    if (!connected_) {
        return;
    }
    ...
    while (decode_mgr_.retrieve_message(buffer)) {
        ...
        for (auto &&cb : handlers_[packet.type()]) {
            if (cb.first) {
                auto future = std::async(std::launch::async, [cb, cb_pkt = std::move(packet),
                                                              this] {
                    cb.second(this, cb_pkt.inner_packet().data(), cb_pkt.inner_packet().length());
                });
                if (block_completion) {
                ...
                cb.second(this, packet.inner_packet().data(), packet.inner_packet().length());
            }
        }
    }
